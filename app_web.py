...
new_player = st.sidebar.text_input("Aggiungi Giocatore:").upper()
if st.sidebar.button("Salva nel Roster"):
    if new_player:
...
if st.sidebar.checkbox("Mostra Roster"):
    st.sidebar.write(roster_list)

# --- MAIN APP ---
st.title("🏀 Basket Scout PRO")

# Selezione Sessione e Squadra
...
# Selezione Giocatore dal Roster e Tempo
col_name, col_time = st.columns([2, 1])
with col_name:
    if roster_list:
        p_name = st.selectbox("Seleziona Tiratore:", roster_list)
    else:
        p_name = st.text_input("Giocatore (Roster vuoto):", "PLAYER 1").upper()
        st.caption("💡 Aggiungi giocatori dalla tendina laterale per velocizzare!")
...
# --- DISEGNO CAMPO ---
    ...
    fig.add_trace(go.Scatter(x=80*np.cos(t_free), y=142.5+80*np.sin(t_free), mode='lines', line_color='white', showlegend=False, hoverinfo='skip'))
    
    # --- ARCO 3 PUNTI COMPLETO (FIBA) ---
    # 1. Le due linee dritte laterali (Corner)
    fig.add_shape(type="line", x0=-220, y0=-47.5, x1=-220, y1=92.5, line_color="white", line_width=2)
    fig.add_shape(type="line", x0=220, y0=-47.5, x1=220, y1=92.5, line_color="white", line_width=2)
    
    # 2. La parte curva dell'arco
    # Calcoliamo l'angolo dove la linea dritta incontra la curva (92.5cm da fondo campo)
    angle = np.arcsin(92.5/237.5)
    t_3pt = np.linspace(angle, np.pi - angle, 60)
    fig.add_trace(go.Scatter(
        x=237.5 * np.cos(t_3pt), 
        y=237.5 * np.sin(t_3pt), 
        mode='lines', line=dict(color='white', width=2), showlegend=False, hoverinfo='skip'
    ))
    
    # --- CANESTRO E TABELLONE ---
    fig.add_shape(type="line", x0=-30, y0=-7.5, x1=30, y1=-7.5, line_color="white", line_width=2)
    fig.add_shape(type="circle", x0=-7.5, y0=-7.5, x1=7.5, y1=7.5, line_color="orange", line_width=2)
...
if st.button("✅ REGISTRA AZIONE", width='stretch', type="primary"):
    ...
    new_shot = {
        "sessione": tipo_sessione, "team": p_team, "player": p_name, "tempo": p_time_input,
...
if st.session_state.shots:
    ...
    with col_rep:
        st.download_button("📥 Scarica Report CSV", data=df.to_csv(index=False).encode('utf-8'), file_name=f"scout_{p_team}.csv", width='stretch')
