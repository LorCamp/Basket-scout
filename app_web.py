...
# --- 4. VISUALIZZAZIONE E REGISTRAZIONE ---
...
if st.button("✅ REGISTRA TIRO", width='stretch', type="primary"):
    ...
    new_shot = {"player": p_name, "x": pos_x, "y": pos_y, "made": True if esito == "Fatto" else False, "type": shot_type}
...
if st.session_state.shots:
    ...
    tot, ok = len(df_stats), len(df_stats[df_stats['made'] == True])
    m1, m2, m3 = st.columns(3)
    m1.metric("Totali", tot)
    m2.metric("Segnati", ok)
    m3.metric("Perc.", f"{(ok/tot*100):.1f}%" if tot > 0 else "0%")
    ...
    with c2:
        ...
        st.download_button("📥 Scarica CSV", data=csv, file_name="scout.csv", width='stretch')
