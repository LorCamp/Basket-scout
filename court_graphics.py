        ...
        for m, c, s in [(True, "#2ecc71", "circle"), (False, "#e74c3c", "x")]:
            ...
            if not mask.empty:
                fig.add_trace(go.Scatter(x=mask['x'], y=mask['y'], mode='markers', 
                                         marker=dict(color=c, size=10, symbol=s), showlegend=False, hoverinfo='skip'))

    # --- MIRINO ATTUALE (Stella Gialla) ---
    fig.add_trace(go.Scatter(x=[px], y=[py], mode='markers', 
                             marker=dict(color='yellow', size=16, symbol='star', line=dict(width=1, color='black')), 
                             showlegend=False, hoverinfo='skip'))

    # --- LAYOUT STATICO ---
    fig.update_layout(
        width=400, height=480, template="plotly_dark",
